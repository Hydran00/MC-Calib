...
continuous_capture = False  # NEW: flag for continuous capture
...
def open_camera(zed, sn, camera_fps=15):
...
class AcquireGui(QWidget):
    ...
    def init_ui(self):
        ...
        self.label = QLabel("Press 'Start' to continuously capture images at 4Hz")
        layout.addWidget(self.label)

        self.button = QPushButton("Start Capture")
        self.button.clicked.connect(self.start_continuous_capture)
    ...
    def start_continuous_capture(self):
        ...
        if not continuous_capture:
            continuous_capture = True
            self.button.setText("Stop Capture")
            self.timer_thread = threading.Thread(target=self.capture_loop, daemon=True)
    ...
    def capture_loop(self):
        global capture_image, image_counter, capture_lock, continuous_capture
        while continuous_capture and not exit_app:
            with capture_lock:
                image_counter += 1
